...
//$pathtonews="/LightNEasy.php?page=news&amp;";
...
//-------------------Pull from DB----------------- 
$query = "SELECT titulo,data,noticia,autor,email,visto,reg,cat FROM noticias ORDER BY reg DESC LIMIT 0, 5";
$roww=fetch_all(dbquery($query));
foreach($roww as $row) {
	$catquery = "SELECT nome FROM newscat WHERE id=".$row[7];
...
//-------------------Show the comments if selected----------------- 
	if($showcomments==true) {
		$comquery = "SELECT * FROM comments WHERE newsid=".$row[6]." ORDER BY time DESC";
